# Lab Algos 2

![Made with Love](https://img.shields.io/badge/Made%20with-Love-pink?style=for-the-badge&logo=data:image/svg%2bxml;base64,)

Laboratorios de Algoritmos y Estructuras de Datos II (CI-2692) de la USB.
Este laboratorio implementará en **Kotlin** algoritmos de ordenamiento, estructuras de datos abstractos, programas recursivos basados en la técnica Divide-and-Conquer, entre otras cosas.

## Grupo de Laboratorio

- Juan Cuevas [@cuevasrja](https://github.com/cuevasrja) (19-10056).
- Luis Isea [@lmisea](https://github.com/lmisea) (19-10175).
